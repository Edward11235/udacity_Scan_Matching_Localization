...
// This NDT function is from an exercise
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			// code below mainly from last lesson exercise
			...
			// Code below comes from an exercise solution
			...
			PointCloudT::Ptr aligned_scan (new PointCloudT);
          	pcl::transformPointCloud (*cloudFiltered, *aligned_scan, transform);

			viewer->removePointCloud("scan");
			// TODO: Change `scanCloud` below to your transformed scan
			renderPointCloud(viewer, aligned_scan, "scan", Color(1,0,0) );
